...
//    float time_taken=0.0;
    ...
    for(window = 0;window<30*24;window++)
    {
        ...
        int a,con=0,den=0;
        filter = (struct bloom *)malloc(sizeof(struct bloom));// Bloom filter corresponding to the topic graph
	float time_taken=0.0;  
       ...
       long long degree=0; 
        ...
        while(1)
        {
            ...
            if((c_in/(24*3600) == day && c_in%(24*3600) >= time*3600)||(c_in/(24*3600) == day+1  && c_in%(24*3600) <= time*3600))
            {
                ...
                //if a is in filter
                if(is_in(a,filter))// User has already occured
                    ;
                else
                {
                    //                   printf("ccccccaaaaaa\n");
		...
		time_taken+= (t2-t1);
                    ...
                    degree+=num_bloom(a);
			   t1=clock();
                        con+= num_bloom(a) - intersection_app(filter,a,num, outgoing_a);
			t2=clock();
			time_taken+=(t2-t1);
                        free(outgoing_a);
                    ...
                    else if(incom[a]>0)
                    {
                       ...
                       t1=clock();
                        con -= intersection_app(filter,a,num,incoming_a);//con = numerator
			t2=clock();
			time_taken+=(t2-t1);
                        free(incoming_a);
